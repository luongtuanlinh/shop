...
<section class="content-header">
    <h1>Sửa màu, size cho sản phẩm</h1>
    <ol class="breadcrumb">
    ...
    <div class="row">
        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">           
            <div class="box box-primary" style="padding: 10px 5px;">
                <form action="{{ route('product.color.edit_amount', ['product' => $productId, 'id' => $data->id]) }}" method="POST" name="form-add-color-product">
                    <input type="hidden" name="_token" value="{{csrf_token()}}" />
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                <label>Kích thước(*)</label>
                                {!! Form::select('size_id', $listSize, $data->size_id, ['class'=>'form-control select2']) !!}
                            </div>
                            <div class="form-group">
                                <label>Màu(*)</label>
                                {!! Form::select('color_id', $listColor, $data->color_id, ['class'=>'form-control select2']) !!}
                            </div>
                            {{-- <div class="form-group">
                                <label>Số lượng(*)</label>
                                <input name="count" type="number" value="{{ $data->count }}" class="form-control" value="{{ old('material') }}" required>
                            </div> --}}
                        </div>
                    </div>
